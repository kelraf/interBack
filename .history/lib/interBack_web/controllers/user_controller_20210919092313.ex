defmodule InterBackWeb.UserController do
  ...
  def login(conn, %{"user" => user}) do
    ...
    case Auth.login user do
      {:ok, user, token} -> json(conn, %{success: true, user: user |> Repo.preload(:storeattendant)  |> Map.from_struct |> IO.inspect |> Map.drop([:__meta__, :inserted_at, :password, :updated_at]), token: token, message: "Login Successful."})

defmodule InterBackWeb.UserController do
  ...
  def login(conn, %{"user" => user}) do
    ...
    case Auth.login user do
      {:ok, user, token} -> json(conn, %{success: true, store_id: user |> Repo.preload(:storeattendant), user: user |> Map.from_struct |> Map.drop([:__meta__, :inserted_at, :password, :updated_at, :storeattendant]), token: token, message: "Login Successful."})
